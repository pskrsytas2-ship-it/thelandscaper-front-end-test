...
const Breadcrumb = ({ items }) => {
  return (
    <div className="flex w-[1152px] justify-end items-center gap-2">
      <div className="flex items-center gap-2">
        {/* Breadcrumb items in RTL order */}
        {items?.map((item, index) => (
          <React.Fragment key={index}>
            <div
              className={`font-['Dubai'] text-sm font-medium ${
                item.active ? 'text-[#147AFC]' : 'text-[#6B7280]'
              }`}
            >
              {item.label}
            </div>
        ...
        {/* Home icon at the end (right side in RTL) */}
        <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
